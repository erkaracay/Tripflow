...
const hasTransferValue = (transfer?: PortalTransferInfo | null) => {
  ...
  return Boolean(
    transfer.pickupTime?.trim() ||
      ...
      transfer.seatNo?.trim() ||
      transfer.compartmentNo?.trim() ||
      transfer.note?.trim()
  )
}
...
const resolveTransferData = () => {
  const outbound = hasTransferValue(travel.value?.transferOutbound) ? travel.value?.transferOutbound ?? null : null
  const ret = hasTransferValue(travel.value?.transferReturn) ? travel.value?.transferReturn ?? null : null
  return { outbound, return: ret, hasAny: Boolean(outbound) || Boolean(ret) }
}
...
<template>
  <div class="space-y-4">
    ...
    <div v-if="printMode" class="space-y-4">
      ...
      >
        <template v-if="resolveTransferData().hasAny">
        ...
        <div class="mt-3 space-y-2 text-sm">
          ...
          <div v-if="formatTime(resolveTransferData().outbound?.pickupTime) !== '—'" class="flex items-start justify-between gap-3 print-row">
            <span class="text-slate-500">{{ t('portal.docs.transferPickupTime') }}</span>
            <span class="text-right font-medium text-slate-800">{{ formatTime(resolveTransferData().outbound?.pickupTime) }}</span>
          </div>
          <div v-if="hasText(resolveTransferData().outbound?.pickupPlace)" class="flex items-start justify-between gap-3 print-row">
            <span class="text-slate-500">{{ t('portal.docs.transferPickupPlace') }}</span>
            <span class="text-right font-medium text-slate-800">{{ valueOrDash(resolveTransferData().outbound?.pickupPlace) }}</span>
          </div>
          <div v-if="hasText(resolveTransferData().outbound?.dropoffPlace)" class="flex items-start justify-between gap-3 print-row">
            <span class="text-slate-500">{{ t('portal.docs.transferDropoffPlace') }}</span>
            <span class="text-right font-medium text-slate-800">{{ valueOrDash(resolveTransferData().outbound?.dropoffPlace) }}</span>
          </div>
          <div v-if="hasText(resolveTransferData().outbound?.vehicle)" class="flex items-start justify-between gap-3 print-row">
            <span class="text-slate-500">{{ t('portal.docs.transferVehicle') }}</span>
            <span class="text-right font-medium text-slate-800">{{ valueOrDash(resolveTransferData().outbound?.vehicle) }}</span>
          </div>
          <div v-if="hasText(resolveTransferData().outbound?.plate)" class="flex items-start justify-between gap-3 print-row">
            <span class="text-slate-500">{{ t('portal.docs.transferPlate') }}</span>
            <span class="text-right font-medium text-slate-800">{{ valueOrDash(resolveTransferData().outbound?.plate) }}</span>
          </div>
          <div v-if="hasText(resolveTransferData().outbound?.driverInfo)" class="flex items-start justify-between gap-3 print-row">
            <span class="text-slate-500">{{ t('portal.docs.transferDriver') }}</span>
            <span class="text-right font-medium text-slate-800">{{ valueOrDash(resolveTransferData().outbound?.driverInfo) }}</span>
          </div>
          <div v-if="hasText(resolveTransferData().outbound?.seatNo)" class="flex items-start justify-between gap-3 print-row">
            <span class="text-slate-500">{{ t('portal.docs.transferSeatNo') }}</span>
            <span class="text-right font-medium text-slate-800">{{ valueOrDash(resolveTransferData().outbound?.seatNo) }}</span>
          </div>
          <div v-if="hasText(resolveTransferData().outbound?.compartmentNo)" class="flex items-start justify-between gap-3 print-row">
            <span class="text-slate-500">{{ t('portal.docs.transferCompartmentNo') }}</span>
            <span class="text-right font-medium text-slate-800">{{ valueOrDash(resolveTransferData().outbound?.compartmentNo) }}</span>
          </div>
          <div v-if="hasText(resolveTransferData().outbound?.note)" class="flex items-start justify-between gap-3 print-row">
            <span class="text-slate-500">{{ t('portal.docs.transferNote') }}</span>
            <span class="text-right font-medium text-slate-800">{{ valueOrDash(resolveTransferData().outbound?.note) }}</span>
          </div>
        </div>

        <div class="mt-4 space-y-2 text-sm">
          <div class="text-xs font-semibold text-slate-500">{{ t('portal.docs.transferReturnTitle') }}</div>
          <div v-if="formatTime(resolveTransferData().return?.pickupTime) !== '—'" class="flex items-start justify-between gap-3 print-row">
            <span class="text-slate-500">{{ t('portal.docs.transferPickupTime') }}</span>
            <span class="text-right font-medium text-slate-800">{{ formatTime(resolveTransferData().return?.pickupTime) }}</span>
          </div>
          <div v-if="hasText(resolveTransferData().return?.pickupPlace)" class="flex items-start justify-between gap-3 print-row">
            <span class="text-slate-500">{{ t('portal.docs.transferPickupPlace') }}</span>
            <span class="text-right font-medium text-slate-800">{{ valueOrDash(resolveTransferData().return?.pickupPlace) }}</span>
          </div>
          <div v-if="hasText(resolveTransferData().return?.dropoffPlace)" class="flex items-start justify-between gap-3 print-row">
            <span class="text-slate-500">{{ t('portal.docs.transferDropoffPlace') }}</span>
            <span class="text-right font-medium text-slate-800">{{ valueOrDash(resolveTransferData().return?.dropoffPlace) }}</span>
          </div>
          <div v-if="hasText(resolveTransferData().return?.vehicle)" class="flex items-start justify-between gap-3 print-row">
            <span class="text-slate-500">{{ t('portal.docs.transferVehicle') }}</span>
            <span class="text-right font-medium text-slate-800">{{ valueOrDash(resolveTransferData().return?.vehicle) }}</span>
          </div>
          <div v-if="hasText(resolveTransferData().return?.plate)" class="flex items-start justify-between gap-3 print-row">
            <span class="text-slate-500">{{ t('portal.docs.transferPlate') }}</span>
            <span class="text-right font-medium text-slate-800">{{ valueOrDash(resolveTransferData().return?.plate) }}</span>
          </div>
          <div v-if="hasText(resolveTransferData().return?.driverInfo)" class="flex items-start justify-between gap-3 print-row">
            <span class="text-slate-500">{{ t('portal.docs.transferDriver') }}</span>
            <span class="text-right font-medium text-slate-800">{{ valueOrDash(resolveTransferData().return?.driverInfo) }}</span>
          </div>
          <div v-if="hasText(resolveTransferData().return?.seatNo)" class="flex items-start justify-between gap-3 print-row">
            <span class="text-slate-500">{{ t('portal.docs.transferSeatNo') }}</span>
            <span class="text-right font-medium text-slate-800">{{ valueOrDash(resolveTransferData().return?.seatNo) }}</span>
          </div>
          <div v-if="hasText(resolveTransferData().return?.compartmentNo)" class="flex items-start justify-between gap-3 print-row">
            <span class="text-slate-500">{{ t('portal.docs.transferCompartmentNo') }}</span>
            <span class="text-right font-medium text-slate-800">{{ valueOrDash(resolveTransferData().return?.compartmentNo) }}</span>
          </div>
          <div v-if="hasText(resolveTransferData().return?.note)" class="flex items-start justify-between gap-3 print-row">
            <span class="text-slate-500">{{ t('portal.docs.transferNote') }}</span>
            <span class="text-right font-medium text-slate-800">{{ valueOrDash(resolveTransferData().return?.note) }}</span>
          </div>
        </div>
        </template>
    ...
    <div v-else-if="activeTab?.kind === 'transfer'" class="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
      ...
      <div class="mt-3 space-y-2 text-sm">
        <div class="text-xs font-semibold text-slate-500">{{ t('portal.docs.transferOutboundTitle') }}</div>
        <div v-if="formatTime(resolveTransferData().outbound?.pickupTime) !== '—'" class="flex items-start justify-between gap-3">
          <span class="text-slate-500">{{ t('portal.docs.transferPickupTime') }}</span>
          <span class="text-right font-medium text-slate-800">{{ formatTime(resolveTransferData().outbound?.pickupTime) }}</span>
        </div>
        <div v-if="hasText(resolveTransferData().outbound?.pickupPlace)" class="flex items-start justify-between gap-3">
          <span class="text-slate-500">{{ t('portal.docs.transferPickupPlace') }}</span>
          <span class="text-right font-medium text-slate-800">{{ valueOrDash(resolveTransferData().outbound?.pickupPlace) }}</span>
        </div>
        <div v-if="hasText(resolveTransferData().outbound?.dropoffPlace)" class="flex items-start justify-between gap-3">
          <span class="text-slate-500">{{ t('portal.docs.transferDropoffPlace') }}</span>
          <span class="text-right font-medium text-slate-800">{{ valueOrDash(resolveTransferData().outbound?.dropoffPlace) }}</span>
        </div>
        <div v-if="hasText(resolveTransferData().outbound?.vehicle)" class="flex items-start justify-between gap-3">
          <span class="text-slate-500">{{ t('portal.docs.transferVehicle') }}</span>
          <span class="text-right font-medium text-slate-800">{{ valueOrDash(resolveTransferData().outbound?.vehicle) }}</span>
        </div>
        <div v-if="hasText(resolveTransferData().outbound?.plate)" class="flex items-start justify-between gap-3">
          <span class="text-slate-500">{{ t('portal.docs.transferPlate') }}</span>
          <span class="text-right font-medium text-slate-800">{{ valueOrDash(resolveTransferData().outbound?.plate) }}</span>
        </div>
        <div v-if="hasText(resolveTransferData().outbound?.driverInfo)" class="flex items-start justify-between gap-3">
          <span class="text-slate-500">{{ t('portal.docs.transferDriver') }}</span>
          <span class="text-right font-medium text-slate-800">{{ valueOrDash(resolveTransferData().outbound?.driverInfo) }}</span>
        </div>
        <div v-if="hasText(resolveTransferData().outbound?.seatNo)" class="flex items-start justify-between gap-3">
          <span class="text-slate-500">{{ t('portal.docs.transferSeatNo') }}</span>
          <span class="text-right font-medium text-slate-800">{{ valueOrDash(resolveTransferData().outbound?.seatNo) }}</span>
        </div>
        <div v-if="hasText(resolveTransferData().outbound?.compartmentNo)" class="flex items-start justify-between gap-3">
          <span class="text-slate-500">{{ t('portal.docs.transferCompartmentNo') }}</span>
          <span class="text-right font-medium text-slate-800">{{ valueOrDash(resolveTransferData().outbound?.compartmentNo) }}</span>
        </div>
        <div v-if="hasText(resolveTransferData().outbound?.note)" class="flex items-start justify-between gap-3">
          <span class="text-slate-500">{{ t('portal.docs.transferNote') }}</span>
          <span class="text-right font-medium text-slate-800">{{ valueOrDash(resolveTransferData().outbound?.note) }}</span>
        </div>
      </div>
      ...
      <div class="mt-5 border-t border-slate-100 pt-4">
        <div class="text-xs font-semibold text-slate-500">{{ t('portal.docs.transferReturnTitle') }}</div>
        <div class="mt-3 space-y-2 text-sm">
          <div v-if="formatTime(resolveTransferData().return?.pickupTime) !== '—'" class="flex items-start justify-between gap-3">
            <span class="text-slate-500">{{ t('portal.docs.transferPickupTime') }}</span>
            <span class="text-right font-medium text-slate-800">{{ formatTime(resolveTransferData().return?.pickupTime) }}</span>
          </div>
          <div v-if="hasText(resolveTransferData().return?.pickupPlace)" class="flex items-start justify-between gap-3">
            <span class="text-slate-500">{{ t('portal.docs.transferPickupPlace') }}</span>
            <span class="text-right font-medium text-slate-800">{{ valueOrDash(resolveTransferData().return?.pickupPlace) }}</span>
          </div>
          <div v-if="hasText(resolveTransferData().return?.dropoffPlace)" class="flex items-start justify-between gap-3">
            <span class="text-slate-500">{{ t('portal.docs.transferDropoffPlace') }}</span>
            <span class="text-right font-medium text-slate-800">{{ valueOrDash(resolveTransferData().return?.dropoffPlace) }}</span>
          </div>
          <div v-if="hasText(resolveTransferData().return?.vehicle)" class="flex items-start justify-between gap-3">
            <span class="text-slate-500">{{ t('portal.docs.transferVehicle') }}</span>
            <span class="text-right font-medium text-slate-800">{{ valueOrDash(resolveTransferData().return?.vehicle) }}</span>
          </div>
          <div v-if="hasText(resolveTransferData().return?.plate)" class="flex items-start justify-between gap-3">
            <span class="text-slate-500">{{ t('portal.docs.transferPlate') }}</span>
            <span class="text-right font-medium text-slate-800">{{ valueOrDash(resolveTransferData().return?.plate) }}</span>
          </div>
          <div v-if="hasText(resolveTransferData().return?.driverInfo)" class="flex items-start justify-between gap-3">
            <span class="text-slate-500">{{ t('portal.docs.transferDriver') }}</span>
            <span class="text-right font-medium text-slate-800">{{ valueOrDash(resolveTransferData().return?.driverInfo) }}</span>
          </div>
          <div v-if="hasText(resolveTransferData().return?.seatNo)" class="flex items-start justify-between gap-3">
            <span class="text-slate-500">{{ t('portal.docs.transferSeatNo') }}</span>
            <span class="text-right font-medium text-slate-800">{{ valueOrDash(resolveTransferData().return?.seatNo) }}</span>
          </div>
          <div v-if="hasText(resolveTransferData().return?.compartmentNo)" class="flex items-start justify-between gap-3">
            <span class="text-slate-500">{{ t('portal.docs.transferCompartmentNo') }}</span>
            <span class="text-right font-medium text-slate-800">{{ valueOrDash(resolveTransferData().return?.compartmentNo) }}</span>
          </div>
          <div v-if="hasText(resolveTransferData().return?.note)" class="flex items-start justify-between gap-3">
            <span class="text-slate-500">{{ t('portal.docs.transferNote') }}</span>
            <span class="text-right font-medium text-slate-800">{{ valueOrDash(resolveTransferData().return?.note) }}</span>
          </div>
        </div>
      </div>
      <p v-if="!resolveTransferData().hasAny" class="mt-3 text-xs text-slate-500">
        {{ t('portal.infoTabs.empty') }}
      </p>
    </div>
