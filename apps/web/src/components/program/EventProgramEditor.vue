...
const backRoute = computed(() => (props.mode === 'guide' ? `/guide/events/${props.eventId}/program` : `/admin/events/${props.eventId}`))
const subtitleKey = computed(() => (props.mode === 'guide' ? 'guide.program.subtitle' : 'admin.program.subtitle'))
...
const mealConfigActivityId = computed(() => (
  activityForm.value.type === 'Meal' && persistedEditingActivity.value?.type === 'Meal'
...
const loadEvent = async () => {
  event.value = await apiGet<EventDto>(`${apiBase.value}/${props.eventId}`)
}

const loadDays = async () => {
  days.value = await apiGet<EventDay[]>(`${apiBase.value}/${props.eventId}/days`)
...
const loadActivities = async (dayId: string) => {
  ...
  try {
    activities.value = await apiGet<EventActivity[]>(
      `${apiBase.value}/${props.eventId}/days/${dayId}/activities`
    )
  } finally {
...
const saveDay = async () => {
  ...
  try {
    if (editingDayId.value) {
      await apiPut(`${apiBase.value}/${props.eventId}/days/${editingDayId.value}`, payload)
    } else {
      await apiPost(`${apiBase.value}/${props.eventId}/days`, payload)
    }
...
const deleteDay = async () => {
  ...
  try {
    await apiDelete(`${apiBase.value}/${props.eventId}/days/${editingDayId.value}`)
...
const moveDay = async (index: number, direction: -1 | 1) => {
  ...
  await Promise.all([
    apiPut(`${apiBase.value}/${props.eventId}/days/${target.id}`, { sortOrder: swap.sortOrder }),
    apiPut(`${apiBase.value}/${props.eventId}/days/${swap.id}`, { sortOrder: target.sortOrder }),
  ])
...
const saveActivity = async () => {
  ...
  try {
    if (editingActivityId.value) {
      await apiPut(`${apiBase.value}/${props.eventId}/activities/${editingActivityId.value}`, payload)
    } else {
      await apiPost(
        `${apiBase.value}/${props.eventId}/days/${selectedDayId.value}/activities`,
        payload
      )
    }
    ...
    const payload = err && typeof err === 'object' ? (err as { payload?: unknown }).payload : null
    const code = payload && typeof payload === 'object' && 'code' in payload
      ? String((payload as { code?: unknown }).code ?? '')
      : ''

    if (code === 'meal_config_exists') {
      pushToast({ key: 'admin.program.meal.errors.mealConfigExists', tone: 'error' })
    } else {
      pushToast({ key: 'errors.generic', tone: 'error' })
    }
  } finally {
...
const deleteActivity = async () => {
  ...
  try {
    await apiDelete(`${apiBase.value}/${props.eventId}/activities/${editingActivityId.value}`)
...
<template>
  <div class="space-y-6">
    <div class="flex flex-col gap-3 rounded-2xl border border-slate-200 bg-white p-4 shadow-sm sm:flex-row sm:items-center sm:justify-between sm:p-6">
      <div>
        <RouterLink class="text-sm text-slate-600 underline" :to="backRoute">
          {{ t('nav.backToEvent') }}
        </RouterLink>
      ...
      <section class="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
        ...
        <div v-else class="mt-4 space-y-3">
          ...
          >
            ...
            <div class="mt-2 text-xs text-slate-500" v-if="activity.notes">
              <RichTextContent :content="activity.notes" />
            </div>
            <div class="mt-3 flex flex-wrap gap-2">
              <button
                v-if="activity.type === 'Meal'"
                class="rounded border border-slate-200 px-2 py-1 text-[10px]"
                type="button"
                @click="openMealReport(activity)"
              >
                {{ t('admin.program.activities.openMealReport') }}
              </button>
    ...
    >
      ...
      >
        ...
        <div class="mt-4 grid min-h-0 flex-1 gap-3 overflow-y-auto pr-1 md:grid-cols-2">
          ...
          <div class="grid gap-1 text-sm md:col-span-2" v-if="activityForm.type === 'Meal'">
          ...
          <ActivityMealConfigEditor
            v-if="activityForm.type === 'Meal'"
            :mode="props.mode"
            :event-id="props.eventId"
            :activity-id="mealConfigActivityId"
            :activity-type="activityForm.type"
            :disabled="savingActivity"
          />
          <div class="grid gap-1 text-sm md:col-span-2" v-if="activityForm.type === 'Program'">
