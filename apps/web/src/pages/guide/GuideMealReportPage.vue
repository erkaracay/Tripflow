...
const loadActivityTitle = async () => {
  ...
  try {
    const schedule = await apiGet<EventSchedule>(`/api/guide/events/${eventId.value}/schedule`)
    const found = schedule.days.flatMap((day) => day.activities).find((item) => item.id === activityId.value)
